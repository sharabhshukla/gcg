   ...
   )
{
   ...
   for( c = colpool->ncols - 1; c >= 0; --c )
   {
      ...
      for( i = 0; i < col->nvars; ++i )
      {
         ...
         if( SCIPisFeasLT(colpool->scip, col->vals[i], SCIPvarGetLbGlobal(GCGpricingVarGetOrigvars(col->vars[i])[0])) ||
             SCIPisFeasGT(colpool->scip, col->vals[i], SCIPvarGetUbGlobal(GCGpricingVarGetOrigvars(col->vars[i])[0])) )
         {
