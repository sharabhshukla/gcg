   ...
   )
{
   ...
   while( GCGvarhistoryHasNext(varhistory) )
   {
      ...
      if( (npricingvars > 0)  && (nnonzeros > 0) )
      {
         ...
         SCIP_CALL( GCGmastersepacutGetExtendedmasterconsCoeff(gcg, mastersepacut, pricingvars, pricingvals, npricingvars,
            blocknr, NULL, &coef) );
      }
   ...
   )
{
   ...
   for( j = 0; j < consdata->naddedcuts; j++ )
   {
      ...
      mastercutrow = GCGextendedmasterconsGetRow(consdata->addedcuts[j]);
      assert(mastercutrow != NULL);
      SCIPdebugMessage("add to active cuts: update row %s from node %lli and add it to active cuts\n",
   ...
   )
{
   ...
   SCIPdebugMessage("init added cuts: remove and free new inactive rows from node %lli of type %i\n",
      SCIPnodeGetNumber(consdata->node), SCIPnodeGetType(consdata->node));
   ...
   {
      SCIPdebugMessage("init consdata: no cuts to store\n");
      consdata->nodestoredcuts = FALSE;
      consdata->addedcutsinit = TRUE;
      return SCIP_OKAY;
   }
   ...
   SCIPdebugMessage("init consdata: store %i cuts for (pseudo)fork/subroot\n",
      consdata->naddedcuts);
...
SCIP_DECL_CONSACTIVE(consActiveMasterbranch)
{
   ...
   SCIPdebugMessage("Activation of node %lli of type %i with cons %s\n", SCIPnodeGetNumber(consdata->node),
...
SCIP_DECL_CONSDELETE(consDeleteMasterbranch)
{
   ...
   if(  (*consdata)->addedcutsinit )
   {
      SCIPdebugMessage("delete master: node stored cuts %i\n", (*consdata)->nodestoredcuts);
...
/** adjusts the firstnewcut bookkeeping of all active masterbranch nodes after a master separator
 *  cut has been removed from the active cuts at the given position
 */
SCIP_RETCODE GCGconsMasterbranchActiveCutRemoved(
   GCG*                  gcg,                /**< GCG data structure */
   int                   removedidx          /**< position the removed cut had in the active cuts */
   )
{
   SCIP* masterprob;
   SCIP_CONSHDLR* conshdlr;
   ...
   for( i = 0; i < conshdlrdata->nstack; ++i )
   {
      SCIP_CONSDATA* consdata;

      if( conshdlrdata->stack[i] == NULL )
         continue;

      consdata = SCIPconsGetData(conshdlrdata->stack[i]);
      assert(consdata != NULL);

      if( consdata->firstnewcut > removedidx )
      {
         assert(consdata->firstnewcut > 0);
         --consdata->firstnewcut;
      }
   }
